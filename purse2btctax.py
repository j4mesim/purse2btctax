...
from pandas import (
    DataFrame, read_csv, to_datetime, set_option, reset_option,
...
def is_purse_file(filepath):
    ...
    return csv_cols != purse_cols, 'mismatch in cols', {
        'csv_cols': csv_cols
    }


def get_purse_mask_sales(df):
    """Get mask for only purchases on Purse.io exports"""

    sale = df.Volume.values < 0
    isfinite = ~df.Amount.isna()
    mask = sale & isfinite

    # check mask against shop
    is_shop = [t.lower() == 'shop' for t in df.Type.values.tolist()]
...
def get_btc_tax_df(df_purse):
    ...
    """
    date = to_datetime(df_purse.Date, utc=True).dt.strftime(
        datetime_fmt_btctax).tolist()
    ...
    price = list(map(operator.truediv, total, volume))
    fee = df_purse.Fee.values.tolist()
    fee_currency = df_purse.iloc[:, 6].values.tolist()  # 2nd "Currency"

    # Build new df
    data = list(zip(date, action, symbol, volume, currency, account, total,
                    price, fee, fee_currency))
...
if __name__ == "__main__":
    ...
    parser.add_argument('fp', type=str, help='Filepath of CSV file of all '
                                             'transactions downloaded from '
                                             'Purse.io')
    year_now = datetime.datetime.now().year
    parser.add_argument('-y', '--year', type=int, default=year_now - 1,
                        help=f'Year to prepare a bitcoin.tax import file ('
                             f'default is previous year, so {year_now-1}).')
    parser.add_argument('-v', '--verbose', action='store_true', default=False,
                        help=f'Display sales.')
    parser.add_argument('-d', '--dry', action='store_true', default=False,
                        help=f'Display sales but dont export, priority over '
                             f'-v/--verbose option.')
    parser.add_argument('-h', '--help', action='help', 
                        help='Help message displayed now.')
    ...
    is_purse_file, msg, d = is_purse_file(args.fp)
    if not is_purse_file:
        raise ValueError(f"csv {os.path.basename(args.fp)} error: {msg} cols " +
    ...
    try:
        ...
        raise ValueError(f"Is '{args.fp}' a Purse.io file?") from e
    ...
    mask_year = df_btc_tax.Date.map(lambda d: d.startswith(str(args.year))).values

    if not mask_year.any() if args.year else not len(df_btc_tax):
        if args.year is None:
            print('0 Purse.io purchases. No file import file created.')
        else:
            print(f'0 Purse.io purchases in the tax year {args.year}. No '
                  f'bitcoin.tax import file created.')
    else:


        if args.year:
            df_btc_tax = df_btc_tax.loc[mask_year, :]
            df_btc_tax = df_btc_tax.reset_index(drop=True).copy()
            fp_btc_tax = args.fp[:-4] + f'_btc_tax_{args.year}.csv'
        else:
            fp_btc_tax = args.fp[:-4] + f'_btc_tax.csv'

        if args.verbose or args.dry:
            print_full(df_btc_tax)
        ...
        if not args.dry:
            ...
            print(f'{n} sales saved to {fp_btc_tax}')
